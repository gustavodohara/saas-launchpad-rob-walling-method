...
- **En la Fase 200, empezá por `/saas_build_mvp_5pasos`** para planificar el MVP: te hace el gate "¿MVP o v1?", te obliga a nombrar el supuesto más riesgoso y —clave— **te frena de codear de entrada** (human automation → no-code → full code, en ese orden; codear solo si se justifica) y recorta el alcance dejando afuera todo lo que se resuelve a mano. Recién después, **si el MVP incorpora IA**, corré `/saas_idea_evaluar_ia`: primero el gate "¿IA es siquiera el approach correcto?" (tener un problema no obliga a usar IA) y después los 16 riesgos de Arvid Kahl. Si el MVP no usa IA, salteálo.
