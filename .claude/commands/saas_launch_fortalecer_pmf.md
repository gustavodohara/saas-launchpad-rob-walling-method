...
2. **Mirá los puentes de memoria de la misma carpeta de idea:**
   ...
   - `1-idea_phase/icp.md` **(ICP — perfil del comprador)** → leelo si existe y **heredá el «para quién»** (no repreguntes el ICP). Este comando **aporta al ICP** la señal más rica de todo el journey: los **clientes reales**, los que **no compraron** y los que **churnearon** — que confirman, refinan o **parten** un perfil. Cuando una conversación revele eso, volcalo a `icp.md` con el **Protocolo de ICP de `CLAUDE.md`** — **avisá y pedí confirmación antes de crear un ICP nuevo (o separar perfiles)**, pedí lo que falte (no lo inventes), y el enriquecimiento de un ICP existente va incluido en el «📝 Guardado en el tablero».
