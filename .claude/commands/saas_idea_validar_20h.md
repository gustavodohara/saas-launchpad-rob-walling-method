...
> **Convención de fuentes (importante).** El cuerpo de este comando —la filosofía del 20h, el **Approach 1** (warm/cold, red vs audiencia, curse of the audience, construcción de red, el caso Jason Cohen/WP Engine, los hangouts, el encuadre de cold outreach) y los **3 principios + preguntas del Mom Test**— sale del video de Rob Walling sobre la validación de 20h por conversaciones, respaldado por `.claude/assets/validar_20h/rob-walling-validacion-20h-conversaciones.md`. El comando **entrelaza otras fuentes ya separadas**, cada una con su asset: la **campaña sistemática de llamadas** (Jason Buckingham, las ~100 llamadas, pre-venta de cheques) → `/saas_idea_campana_llamadas`; el **detalle del Approach 2 / landing** (ejemplos, umbrales de opt-in) → `/saas_idea_campana_landing` y el overview en `.claude/assets/validar_2_20_200/`; las **13 red flags** y la guía de "cuánto tiempo lleva validar" → `.claude/assets/_compartido/rob-walling-validation-red-flags.md`; y el concepto **commitment & advancement ("las 3 monedas")** de la sección "¿cómo sé si mi red es honesta?" viene del **libro *The Mom Test*** (Rob Fitzpatrick), no de este video, respaldado por `.claude/assets/_compartido/the-mom-test-rob-fitzpatrick.md` (resumen citable) y su PDF al lado. Toda la maquinaria de tablero, gates, espejo a Drive y handoffs es scaffolding del sistema.
...
Recomendá leer **"The Mom Test"** (*El test de la mamá*, de Rob Fitzpatrick — barato en Kindle/Audible/papel). Es clave si vas a tener estas conversaciones. El **método completo y citable** (las 3 reglas, la tabla de buenas/malas preguntas con su arreglo, las 3 monedas, VPDPP, segmentación quién-dónde, símbolos de notas) vive en `.claude/assets/_compartido/the-mom-test-rob-fitzpatrick.md` (PDF al lado) — consultalo si necesitás profundizar más allá de este resumen. Resumen de los **3 principios**:
